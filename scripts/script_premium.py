...
def api_requisition(user_message):
    response = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "Dada uma sentença e seus respectivos marcadores sobre o mesmo alvo de opinião responda apenas com o caracter (-1) se ela possui conotação negativa, (0) se for neutra ou (1) se for positiva"},
...
for sentence in sentence_corpus:
    ...
    while request_200 == False:
        try:
            ...
            df_gpt.to_excel('./outputs/new_prompt_output2.xlsx', index=False)
