...
df = pd.read_excel('./inputs/entities_entradas.xlsx')
...
def api_requisition(user_message):
    prompt_system = "Dada a seguinte sentença, responda no formato ['entidade1', 'entidade2'] a(s) entidade(s) nomeada(s) presente(s) na sentença."
...
for sentence in sentence_corpus:
    ...
    while request_200 == False:
        try:
            ...
            df_gpt.to_excel('./outputs/entities_output.xlsx', index=False)
